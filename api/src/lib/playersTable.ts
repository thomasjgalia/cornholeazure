...
// shared with the sister golf app, so the entity shape here must stay in
// sync with golfazure's equivalent module (api/src/lib/playersTable.ts).
// Cornhole doesn't have an admin/role concept of its own, but is_admin is
// still read/written here so it round-trips untouched for golf's benefit.
